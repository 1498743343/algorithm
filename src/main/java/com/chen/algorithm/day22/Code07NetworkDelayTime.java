 ...
 * <a href="https://leetcode.cn/problems/network-delay-time">测试链接</a>
 *
 * @author chenzihan
 * @date 2022/08/24
 */
public class Code07NetworkDelayTime {
	...
	public static int networkDelayTime1(int[][] times, int n, int k) {
		...
		for (int[] info : times) {
			list.get(info[0]).add(new int[]{info[1], info[2]});
		}
		PriorityQueue<int[]> heap = new PriorityQueue<>(Comparator.comparingInt(array -> array[1]));
		...
		while (!heap.isEmpty()) {
			...
			for (int[] info : list.get(cur)) {
				heap.add(new int[]{info[0], info[1] + distance});
			}
		}
		return num < n ? -1 : max;
	}
